...
<?php 
if ($num_projects = Project::count_items('projects')) {
  if (isset($_GET['page'])) {
    $paginate = $paginate = new Paginate($num_projects, 25, $_GET['page']);
  } else {
    $paginate = new Paginate($num_projects, 25, 1);
  }
...
foreach ($all_projects as $project) {
  ...
  <tr class='admin__table--row'>
    ...
    <td class='admin__table--entry'><a href='/blog/projects/{$project->id}'>{$project->title}</a></td>
